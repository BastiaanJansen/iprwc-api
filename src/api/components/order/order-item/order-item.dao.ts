...
export const create = async (orderID: number, dto: CreateOrderItemDTO): Promise<OrderItem> => {
...
export const findAll = async (): Promise<DBFindAllResponse<OrderItem[]>> => {
    const builder: SelectQueryBuilder<OrderItem> = getRepository(OrderItem).createQueryBuilder("item");
...
export const update = async (id: number, dto: UpdateOrderItemDTO): Promise<OrderItem> => {
    const product = await getRepository(Product).findOne(dto.product);

    if (dto.product && !product) throw new BadRequestException("Product does not exist");
